...
function Message(request, response, message) {
    ...
    this._message = (Array.isArray(message) && message.length > 0) ? message : [];
    return this;
}
...
Message.method('setMessage', function (message) {
    this._message = (Array.isArray(message) && message.length > 0) ? message : this._message;
    return this;
});

/**
 * Accessor method to allow for the retrieval of the message field.
 *
 * @return {Array} representative of the hypermedia message. An {Array} will always be returned. The {Array} may be
 * empty if arguments supplied to the constructor were incorrect.
 */
Message.method('getMessage', function () {
...
/**
 * Allows for checking if a Message is meant for a particular request/response exchange.
 * Example: Message.isForExchange('entry', '200')
 *
 ...
 * @return true, if the Message is meant for the request/response pair; false, otherwise.
 */
Message.method('isForExchange', function (request, response) {
