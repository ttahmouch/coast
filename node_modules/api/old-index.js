...
var generator = common.generator,
    ...
    hypermediaAfforance = xhn.affordance,
...
var seperator = path.sep,
    folder = __dirname + seperator + 'gen' + seperator;

var api = resources((fs.existsSync(folder + 'api.json')) ? JSON.parse(fs.readFileSync(folder + 'api.json', 'utf8')) : undefined),
    rels = relations(),
    apigen = generator()
        .setAffordances(api.getAffordances(true))
        .setDelegate(function (resource, method, status, affordances) {
            ...
            var hypermedia = hypermediaCollection(statusString);
            ...
            function finish() {
                try {
                    ...
                    /**
                     * Reintroduce NavAL-XML eventually.
                     */
                    fs.writeFileSync(resourcePath + file, encoder((xml) ? naval : naval).encode(hypermedia));
                } catch (e) {
            ...
            function chooseAction() {
                ...
                program.choose(affordances, function (action) {
                    if (action === 0) {
                        ...
                        var link = hypermediaAfforance(id, null, method, resource);
                        hypermedia.addAffordance(link);
...
program
    .command('ls')
    .description('List relevant information from your API specification or from RFCs.')
    .action(function () {
        console.log('List relevant information from your API specification or from RFCs. Utilize option flags.');
    });
...
fs.writeFileSync(folder + 'api.json', JSON.stringify(api, null, '    '));
fs.writeFileSync(folder + 'actions.json', JSON.stringify(api.getActionPossibilities(), null, '    '));
