 ...
 *
 * @param affordances should be an Array of Affordance instances. This can be provided by an instance of a Resources
 * collection by invoking the list method. This array indicates all action possibilities that may be taken within the
 * API specification. That implies that an Affordance represents a method that may be invoked on a resource so long as
 * they are both specified in the API specification.
 *
 * @return undefined
 */
Method.method('generate', function (statuses, affordances) {
    var returns = Array.isArray(statuses) ? statuses : this._returns,
        isTheReturnsPropertyArray = (returns === this._returns);

    /**
     * If the argument passed in is not an Array, or
     * If the argument passed in is an Array with a length of zero,
     * then don't bother generating because there are no other affordances.
     */
    if (!Array.isArray(affordances) || affordances.length <= 0) {
    ...
    /**
     * Iterate over the entire returns array
     */
    for (var ret in returns) {
        /**
         * Dereference with the index, and augment the ret variable with a status code value.
         */
        ret = returns[ret];

        /**
         * If the reference in the returns variable is not the _returns field, meaning it's a custom set of status
         * codes, then check to the see if the status code is defined in this Method specification.
         * If it is defined, then generate a hypermedia response for it.
         */
        if (isTheReturnsPropertyArray || this._returns.lastIndexOf(ret) !== -1) {
            console.log(this._method.toString() + ' ' + ret.toString());
            /**
             * Dump a list of possible affordances defined in the API specification. These affordances should be
             * chosen from and have relations defined for each context.
             */
        } else {
            console.log(ret + ' is not a valid status code defined in the API specification.');
        }
    }
});
 ...
 * @return undefined
 */
Resource.method('generate', function (affordances) {
    var methods = this._methods;
     ...
     */
    if (methods.length <= 0) {
        console.log('There are no valid methods specified in your resource. Please put some methods.');
        return;
    }
 ...
 * @return undefined
 */
Resources.method('generate', function (affordances) {
    var resources = this._resources;
    ...
    for (var res in resources) {
         ...
         */
        res.generate(affordances);
    }
});

/**
 * list retrieves a two-dimensional Array of all possible Resource, Method, and Arguments (Media Type) defined
 * in your API specification. This is intended to be used as the Array of choices for the user when they are required
 * to select a possible affordance at a given state.
 *
 ...
 *
 * @return Array representing all possible combinations of Resource, Method, and Arguments defined within the API
 * specification. This Array may be empty if no Resources or Methods are defined.
 */
        ...
        if (!!met) {
            ...
            if (!!res) {
                ...
                if (!!met) {
                     ...
                     * array of status codes. The list method retrieves all Affordances available.
                     */
                    met.generate(ret, api.list(!program.debug));
                } else {
            ...
            if (!!res) {
                /**
                 * Generate responses for a particular Resource specification. The list method retrieves all
                 * Affordances available.
                 */
                res.generate(api.list(!program.debug));
            } else {
                console.log('A valid, existent resource from your API should be entered. An example: /uri');
            }
        } else {
            /**
             * Generate responses for the entire API specification. The list method retrieves all Affordances available.
             */
            api.generate(api.list(!program.debug));
        }
    });
...
program
    ...
    .action(function () {
        /**
         * Call the list method on the Resources collection, and pass in the quiet flag that could be set in the CLI.
