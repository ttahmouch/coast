...
Resources.method('getActionPossibilities', function () {
     ...
     */
    var resources = this._resources,
        affordances = xhn.affordances('affordances');
    ...
    for (var res in resources) {
        ...
        for (var met in methods) {
            ...
            met = methods[met];
            /**
             * Create an instance of an HC with the respective information from the method and resource.
             * @type {HC}
             */
            var affordance = xhn.hc();
            /**
             * Explicitly undefine the relation property because IANA relations are context-sensitive.
             */
            affordance._relation = undefined;
            /**
             * Explicitly set the URI and Method on the affordance. These should always be defined.
             */
            affordance._uri = res._uri;
            affordance._method = met._method;
            /**
             * An ID on a Method instance may be either a string instance or undefined.
             */
            if (!!met._id) {
                affordance._id = met._id;
            }
            /**
             ...
             */
            if (!!met._arguments) {
                affordance._metadata.addField('Content-Type', met._arguments);
            }
